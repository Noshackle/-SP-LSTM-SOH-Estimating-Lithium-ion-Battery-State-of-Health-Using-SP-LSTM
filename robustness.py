...
class RobustnessAnalyzer:
    ...
    def _extract_features_for_battery(self, test_battery_id, noise_std=0):
        ...
        for i in range(len(cycles)):
            ...
            feats = self.extractor.extract(cycle_data, micro_vars)
            feat_vec = [
    ...
    def run_analysis(self, test_battery_id, noise_levels=[0, 0.005, 0.01, 0.02], n_repeats=5):
        ...
        for nl in noise_levels:
            ...
            for rep in range(repeats):
                ...
                # SPM 特征预处理（与 main.py 一致）
                # Step 1: SPM 特征做差分 (相对变化率)
                spm_indices = [3, 4, 5, 6]
                for idx in spm_indices:
                    col = X_raw[:, idx].copy()
                    ...
                    for k in range(1, len(col)):
                        if abs(col[k-1]) > 1e-10:
                            ...
                            delta[k] = 0.0
                    X_raw[:, idx] = delta
                # Step 2: SPM 特征 Per-Battery Z-Score
                for idx in spm_indices:
                    mu = np.mean(X_raw[:, idx])
                    sigma = np.std(X_raw[:, idx])
                    if sigma > 1e-10:
                        X_raw[:, idx] = (X_raw[:, idx] - mu) / sigma
                    else:
                        X_raw[:, idx] = 0.0
    ...
    def plot_results(self, results, save_dir='plots/robustness', fig_label=None):
        ...
        fname = 'noise_robustness.png'
        if fig_label:
            try:
                chap = fig_label.split('-')[0].split()[-1]
                fname = f"第{chap}章_{fig_label.replace(' ', '')}_鲁棒性分析.png"
            except: pass
